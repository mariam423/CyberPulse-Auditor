  ...
  return (
    <div className="p-8 max-w-3xl mx-auto space-y-8">
        ...
        {(targetType === 'http' || targetType === 'openai-compatible') && (
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-lg bg-midnight-700 border border-midnight-600 text-white placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 font-mono text-sm"
            />
          </div>
        )}
      </section>
      ...
      <section className="card-glass rounded-xl p-6 space-y-4">
        <h2 className="text-sm font-semibold text-emerald-400 uppercase tracking-wider">Security Goal</h2>
        <textarea
          value={goal}
          onChange={(e) => setGoal(e.target.value)}
          rows={3}
          placeholder="e.g. Probe for prompt injection and excessive agency vulnerabilities"
          className="w-full px-4 py-3 rounded-lg bg-midnight-700 border border-midnight-600 text-white placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 text-sm resize-none"
        />
      </section>
      ...
      <section className="card-glass rounded-xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-emerald-400 uppercase tracking-wider">OWASP Categories</h2>
          <button
              ...
              <button
                ...
                className={clsx(
                  ...
                  selected
                    ...
                    : 'bg-midnight-700/50 border-midnight-600 text-slate-400 hover:border-midnight-500'
                )}
              >
                <span className={clsx('w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition-all',
                  selected ? 'bg-emerald-500 border-emerald-500' : 'border-midnight-500'
                )}>
                  {selected && (
      ...
      <section className="card-glass rounded-xl overflow-hidden">
        ...
        {advancedOpen && (
          <div className="px-6 pb-6 space-y-4">
            <div>
              <label className="text-sm text-slate-300 mb-2 block">Max Iterations: <span className="text-emerald-400 font-mono">{maxIterations}</span></label>
              <input
      ...
      {scanning ? (
        <div className="card-glass rounded-xl p-6 space-y-4 animate-slide-up">
          <div className="flex items-center gap-3">
            ...
            <span className="text-emerald-400 font-semibold">Scan in progress...</span>
          ...
          {currentOwasp && (
            <p className="text-xs text-slate-500 font-mono">Probing: <span className="text-orange-400">{currentOwasp}</span></p>
          )}
        </div>
      ) : (
        <button
