...
import {
  Terminal, Apple, Monitor, Download, Shield, BookOpen,
...
function CodeBlock({ commands, shell }: { commands: string[]; shell: string }) {
  ...
  return (
    <div className="rounded-lg bg-obsidian-950 border border-obsidian-700 overflow-hidden">
      ...
      <pre className="p-4 text-sm text-amber-300/90 font-mono overflow-x-auto leading-relaxed">
        {commands.join('\n')}
      </pre>
    </div>
  );
...
export default function OnboardingPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-8 lg:space-y-10">
      {/* Header */}
      <div>
        <div className="flex items-center gap-3">
          <BookOpen className="w-6 h-6 sm:w-7 sm:h-7 text-amber-400" />
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white">Getting Started</h1>
      ...
      <section className="space-y-5">
            ...
            {(Object.keys(PLATFORM_META) as Platform[]).map((p) => {
              ...
              return (
                <button
                  ...
                  className={clsx(
                    'flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium border transition-all',
                    platform === p
        ...
        <p className="text-xs text-slate-500 -mt-1">
          Prefer the full manual setup? Follow the steps below.
        </p>

        <div className="space-y-6">
          {steps.map((step) => (
            <div key={step.title} className="card-glass rounded-xl p-4 sm:p-6 space-y-4">
              <h3 className="text-sm font-semibold text-amber-400">{step.title}</h3>
          ...
          <div className="px-6 pb-6 space-y-3">
            {CLI_REFERENCE.map(({ cmd, desc }) => (
              <div key={cmd} className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-1 md:gap-4 items-baseline">
                <code className="text-xs text-amber-300/90 font-mono break-all">{cmd}</code>
                <span className="text-xs text-slate-500">{desc}</span>
              </div>
            ))}
            <p className="text-xs text-slate-500 pt-2 border-t border-obsidian-700 mt-4">
