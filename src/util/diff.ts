...
export function parseUnifiedDiff(diff: string): FileDiff | null {
  ...
  while (i < lines.length) {
    ...
    while (i < lines.length && !lines[i]?.startsWith('@@')) {
      if (lines[i]) hunk.body.push(lines[i]!);
      i++;
    }
    hunks.push(hunk);
  }
...
function computeHunks(oldLines: string[], newLines: string[]): DiffHunk[] {
  ...
  let i = oldLen;
  let j = newLen;
  const edits: Array<{ type: 'keep' | 'delete' | 'insert'; oldLine?: number; newLine?: number; content: string }> = [];
  ...
  function flushHunk(start: number, end: number, oStart: number, nStart: number): void {
    if (end <= start) return;
    const body = edits.slice(start, end).map((e) => {
      if (e.type === 'keep') return ` ${e.content}`;
      if (e.type === 'delete') return `-${e.content}`;
      return `+${e.content}`;
    });
    hunks.push({
      oldStart: oStart,
