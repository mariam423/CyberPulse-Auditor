...
async function pick(prompt, options, labels) {
...
/** Free-text input. */
async function input(prompt, fallback) {
    console.log(chalk.bold.cyan(`\n${prompt}`));
    if (fallback)
        console.log(chalk.dim(`  [Enter] = "${fallback}"`));
    return new Promise((resolve) => {
        const q = rl();
    ...
    return new Promise((resolve) => {
        ...
        q.question(chalk.gray('\n  Categories (e.g. 1 3 5, or Enter for all): '), (answer) => {
            ...
            const selected = answer
                ...
                .map((n) => cats[n - 1].id);
            resolve(selected.length > 0 ? selected : cats.map((c) => c.id));
        });
    });
}
const MODEL_CHOICES = ['openai', 'anthropic', 'ollama'];
const TARGET_TYPES = ['http', 'openai-compatible', 'python-fn'];
/**
...
export async function runInteractive() {
    ...
    // 1. Security Goal
    const goal = await input('1. Security Goal', 'Probe for prompt injection, excessive agency, and system prompt leakage vulnerabilities');
    // 2. Target Type
    const targetType = await pick('2. Target Type', TARGET_TYPES, {
    ...
    const modelName = modelProvider === 'openai'
    ...
    const apiKey = modelProvider !== 'ollama'
        ? await input(`API Key for ${modelProvider} (Enter = from OPENAI_API_KEY env var)`, '')
        : '';
    // 6. OWASP categories
    const owaspIds = await selectOwasp();
    // 7. Iterations
    const maxIterations = parseInt(await input('6. Max attack iterations (Enter = 3)', '3'), 10);
    ...
    console.log(`  Target:        ${chalk.cyan(targetType)} — ${chalk.white(targetUrl || pythonFn)}`);
    ...
    await runAudit({
        ...
        model: modelName,
        ...(apiKey ? { apiKey } : {}),
    }, 'text', process.env.CYBERPULSE_DB_PATH ?? 'data/cyberpulse.db');
}
