 ...
 */
export function buildBlockingSchema(attackPayload, _legitimateExample) {
    ...
    const conditions = suspiciousPhrases.map((phrase) => `val.toLowerCase().includes(${JSON.stringify(phrase.toLowerCase())})`);
 ...
 */
export function generateDiff(file, oldLines, newLines) {
    if (oldLines.join('\n') === newLines.join('\n'))
        return '';
...
function computeDiffHunks(oldLines, newLines) {
    ...
    while (editI < edits.length) {
        // Find next change
        while (editI < edits.length && edits[editI]?.type === 'keep')
            editI++;
 ...
 */
export function createCodePatch(opts) {
...
//# sourceMappingURL=code-patch.js.map