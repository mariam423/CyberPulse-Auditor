...
export class Validator extends Agent {
    ...
    async invoke(input, _attempt) {
        ...
        const verdict = passRate < 0.3 ? 'closed' : passRate > 0.7 ? 'open' : 'inconclusive';
    ...
    buildPayloads(base, runMutations) {
        const payloads = [base];
        if (!runMutations)
            return payloads;
    ...
    async runPayloadMultiple(payload, systemPrompt) {
        const turns = [];
        if (systemPrompt) {
     ...
     */
    isAttackBlocked(response, _payload) {
    ...
    buildEvidence(attempts, passRate) {
        const lines = [];
